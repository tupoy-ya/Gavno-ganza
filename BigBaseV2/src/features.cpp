...
namespace big::features
{
	...
	namespace online {
		void features::online::draw_extra_info()
		{
			if (CNetGamePlayer* net_player = g_pointers->m_GetNetPlayer(online::selectedPlayer))
			{
				...
				if (ImGui::BeginChild("Player Menu", ImVec2(700, 700), true, window_flags))
				{
					...
					*/
					if (ImGui::Button("Give rid"))
					{
						g_fiber_pool->queue_job([]
							{
								LOG(INFO) << features::online::ReturnPlayerRockstarID(online::selectedPlayer);
							});
					}
					...
					*/
					...
					if (ImGui::Button("Host Kick"))
					{
						g_fiber_pool->queue_job([]
							{
								NETWORK::NETWORK_SESSION_KICK_PLAYER(online::selectedPlayer);
							});
					}
					if (ImGui::Button("Remote_Ring"))
					{
						g_fiber_pool->queue_job([]
						{
							AUDIO::PLAY_PED_RINGTONE("Remote_Ring", PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(online::selectedPlayer), 1);
						});
					}
					if (ImGui::Button("Remote_Ring"))
					{
		...
		int features::online::ReturnPlayerRockstarID(Player PlayerHandle)
		{
		...
		Cam antiCrashCam;
		void features::online::EnableDisableAntiCrashCamera()
		{
			if (CAM::DOES_CAM_EXIST(antiCrashCam))
			{
				CAM::SET_CAM_ACTIVE(antiCrashCam, false);
				CAM::RENDER_SCRIPT_CAMS(0, 1, 10, 0, 0, 0);
			...
			else
			{
				...
				CAM::RENDER_SCRIPT_CAMS(1, 1, 1, 0, 0, 0);
...
/*		void features::online::AirstrikePlayer() {
	...
	namespace self {
		...
		bool features::self::pron_mod_bool = true;
	...
	void features::self::pron_mod()
	{
		if (pron_mod_bool)
		{
			...
			{
				PAD::DISABLE_CONTROL_ACTION(0, proneKey, true);
				PAD::DISABLE_CONTROL_ACTION(0, crouchKey, true);
				if (!HUD::IS_PAUSE_MENU_ACTIVE())
				{
					if (PAD::IS_DISABLED_CONTROL_JUST_PRESSED(0, crouchKey) && !proned)
					{
						STREAMING::REQUEST_ANIM_SET("move_ped_crouched");
						STREAMING::REQUEST_ANIM_SET("MOVE_M@TOUGH_GUY@");
						if (crouched && !proned)
						{
							PED::RESET_PED_MOVEMENT_CLIPSET(ped, 1);
							PED::RESET_PED_STRAFE_CLIPSET(ped);
							PED::SET_PED_MOVEMENT_CLIPSET(ped, "MOVE_M@TOUGH_GUY@", 0.5);
							crouched = false;
						}
						if (!crouched && !proned)
						{
							PED::SET_PED_MOVEMENT_CLIPSET(ped, "move_ped_crouched", 0.55);
							PED::SET_PED_STRAFE_CLIPSET(ped, "move_ped_crouched_strafing");
							crouched = true;
						}
						if (PAD::IS_DISABLED_CONTROL_JUST_PRESSED(0, proneKey) && !crouched && !PED::IS_PED_IN_ANY_VEHICLE(ped, true))
						{
							if (proned)
							{
								TASK::CLEAR_PED_TASKS(ped);
								auto me = ENTITY::GET_ENTITY_COORDS(ped, true);
								ENTITY::SET_ENTITY_COORDS(ped, me.x, me.y, me.z - 0.5, true, true, true, TRUE);
								proned = false;
								if (!proned)
								{
									STREAMING::REQUEST_ANIM_SET("move_crawl");
									while (!STREAMING::HAS_ANIM_SET_LOADED("move_crawl"))
									{
										TASK::CLEAR_PED_TASKS_IMMEDIATELY(ped);
										proned = true;
										if (TASK::IS_PED_SPRINTING(ped) || TASK::IS_PED_SPRINTING(ped) || TASK::IS_PED_SPRINTING(ped) > 5)
										{
											TASK::TASK_PLAY_ANIM(ped, "move_jump", "dive_start_run", 8.0, 1.0, -1, 0, 0.0, 0, 0, 0);
										}
									}
								}
							}
				...
				if (proned)
				{
					TASK::CLEAR_PED_TASKS_IMMEDIATELY(ped);
					auto pos = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true);
					TASK::TASK_PLAY_ANIM_ADVANCED(ped, "move_crawl", "onfront_fwd", pos.x, pos.y, pos.z, 0.0, 0.0, ENTITY::GET_ENTITY_HEADING(ped), 1.0, 1.0, 1.0, 46, 1.0, 0, 0);
				}
			}
		}
	}
